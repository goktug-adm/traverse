...
import { monumentFor, featuredCodes } from './monuments/catalog.js';
...
const globe = new Globe(document.getElementById('globe'))
  ...
  .polygonSideColor(() => 'rgba(40, 70, 120, 0.35)')
  .polygonStrokeColor(() => 'rgba(150, 190, 255, 0.35)')
  ...
  .objectLabel(d => d.type === 'monument'
    ? `<div class="globe-tooltip"><b>🏛️ ${d.entry.n}</b><div class="t-sub">${[d.entry.city, d.country].filter(Boolean).join(', ')} — click to view in 3D</div></div>`
...
globe.globeMaterial().emissive = new THREE.Color(0x061224);
globe.globeMaterial().emissiveIntensity = 0.35;
...
function capColor(f) {
  ...
  if (isSel) return 'rgba(76, 201, 240, 0.75)';
  if (store.isCountryVisited(r.a2)) return isHov ? 'rgba(46, 196, 166, 0.95)' : 'rgba(46, 196, 166, 0.72)';
  return isHov ? 'rgba(120, 160, 220, 0.6)' : 'rgba(66, 90, 130, 0.42)';
}
...
function makeObject3D(d) {
  if (d.type === 'monument') {
    const g = buildMonument(d.entry.b, d.entry.p);
    g.scale.setScalar(d.entry.f ? MON_SCALE * 1.15 : MON_SCALE);
...
function refreshObjects() {
  const data = [];
  if (showMonuments) {
    const codes = showAllMonuments ? countries.map(r => r.a2) : featuredCodes.filter(c => byA2.has(c));
    for (const code of codes) { const r = byA2.get(code); if (r) data.push(monDatum(r)); }
...
searchInput.addEventListener('change', () => {
  ...
  if (!q) return;
  const rec = countries.find(r => r.name.toLowerCase() === q) ||
    countries.find(r => r.name.toLowerCase().startsWith(q)) ||
    countries.find(r => r.name.toLowerCase().includes(q));
...
btnAdd.addEventListener('click', () => {
  addPlaceArmed = !addPlaceArmed;
  btnAdd.classList.toggle('armed', addPlaceArmed);
  btnAdd.textContent = addPlaceArmed ? '🎯 Click the globe…' : '📍 Add place';
...
function beginPlace(lat, lng, a2) {
  ...
  addPlaceArmed = false;
  btnAdd.classList.remove('armed');
  btnAdd.textContent = '📍 Add place';
...
function renderWelcome() {
  ...
  panel.innerHTML = `
    ...
    <div class="chips">${featuredCodes.filter(cd => byA2.has(cd)).map(cd => {
      ...
      return `<span class="chip" style="background:rgba(247,179,43,.12);border-color:rgba(247,179,43,.4);color:#f7b32b"
        data-action="view-mon-of" data-a2="${cd}">${r.meta.flag} ${esc(m.n)}</span>`;
    }).join('')}</div>
  `;
}
